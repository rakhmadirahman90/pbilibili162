...
const Players: React.FC<{ initialFilter?: string }> = ({
  ...
  // Fungsi fetch dibungkus useCallback agar stabil
  ...
  /**
   * PERBAIKAN UTAMA: LOGIKA UNIFIKASI DATA (ANTI-DUPLIKASI)
   * Menggunakan Map untuk memastikan satu pendaftaran_id hanya muncul satu kali.
   */
  const processedPlayers = useMemo(() => {
    ...
    dbPlayers.forEach((p) => {
      ...
      // Gunakan ID pendaftaran sebagai kunci unik, jika tidak ada gunakan ID stats
      const uniqueKey = info.id || p.id;

      // Jika atlet sudah ada di Map, kita bisa bandingkan poinnya atau abaikan saja
      ...
      if (dbCategory) {
        ...
        if (
          categoryRaw.includes('MUDA') ||
          ['U-9', 'U-11', 'U-13', 'U-15', 'U-17', 'U-19'].some((u) =>
            categoryRaw.includes(u)
          )
        ) {
          ageGroup = 'Muda';
        }
    ...
    // Ubah Map kembali ke Array dan urutkan berdasarkan poin tertinggi
    ...
    <section id="atlet" className="py-24 bg-[#050505] text-white min-h-screen relative overflow-hidden font-sans">
      ...
      {selectedPlayer && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-8">
          ...
          <div className="relative bg-[#0f0f0f] border border-white/10 w-full max-w-5xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
            <div className="flex flex-col md:flex-row h-full">
              <div className="w-full md:w-[45%] relative bg-gradient-to-b from-zinc-800 to-black overflow-hidden">
                {selectedPlayer.img ? (
                ...
                <div className="absolute top-8 left-8 bg-blue-600 px-6 py-2 rounded-2xl border border-white/20">
              ...
              <div className="flex-1 p-8 md:p-14 flex flex-col justify-center relative">
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="px-4 py-1.5 bg-blue-600 text-white rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><ShieldCheck size={14} /> {selectedPlayer.ageGroup.toUpperCase()}</span>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white/5 border border-white/5 p-6 rounded-[2rem] md:col-span-1 col-span-2">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input type="text" placeholder="Cari nama atlet..." className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl py-4 pl-12 pr-4 text-xs outline-none focus:border-blue-600 transition-all" onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </div>
        ...
        <div className="flex bg-zinc-900/50 p-1.5 rounded-full border border-zinc-800 w-fit mb-16 backdrop-blur-sm">
          {[
            ...
            <button key={tab.id} onClick={() => setCurrentAgeGroup(tab.id)} className={`px-8 py-3.5 rounded-full text-[11px] font-black transition-all flex items-center gap-3 ${currentAgeGroup === tab.id ? 'bg-blue-600 text-white' : 'text-zinc-500 hover:text-white'}`}>
              {tab.label} <span className={`text-[10px] px-2 py-0.5 rounded-lg ${currentAgeGroup === tab.id ? 'bg-white/20 text-white' : 'bg-zinc-800 text-zinc-600'}`}>{tab.count}</span>
            </button>
          ))}
        </div>
          ...
          <div className="relative group/slider">
            {filteredPlayers.length > 0 ? (
              ...
              >
                {filteredPlayers.map((player) => (
                  <SwiperSlide key={player.id}>
                    <div onClick={() => setSelectedPlayer(player)} className="group cursor-pointer relative aspect-[3/4.2] rounded-[3rem] overflow-hidden bg-zinc-900 border border-zinc-800 hover:border-blue-600/50 transition-all duration-700">
                      {player.img ? (
                      ...
                      <div className="absolute bottom-8 left-8 right-8 transform group-hover:-translate-y-2 transition-transform duration-500">
                        ...
                        <h3 className="text-2xl font-black uppercase italic mb-4 leading-tight group-hover:text-blue-500 transition-colors line-clamp-2">{player.name}</h3>
              ...
              <div className="py-32 text-center">
                ...
                <p className="text-zinc-600 font-black uppercase italic tracking-widest">Atlet tidak ditemukan dalam kategori ini</p>
                <button onClick={() => {setCurrentAgeGroup('Semua'); setSearchTerm('');}} className="mt-4 px-6 py-2 bg-blue-600 rounded-full text-xs font-bold hover:bg-blue-700 transition-all">Lihat Semua Atlet</button>
              </div>
            )}
            <button ref={prevRef} className="absolute -left-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center opacity-0 group-hover/slider:opacity-100 transition-all hover:bg-blue-600 text-white shadow-xl"><ChevronLeft size={28} /></button>
            <button ref={nextRef} className="absolute -right-6 top-1/2 -translate-y-1/2 z-20 w-14 h-14 bg-zinc-900/80 backdrop-blur-md border border-white/10 rounded-full flex items-center justify-center opacity-0 group-hover/slider:opacity-100 transition-all hover:bg-blue-600 text-white shadow-xl"><ChevronRight size={28} /></button>
          </div>
        )}
      </div>
    </section>
