...
export default function KasManager() {
  ...
  const exportToPDF = async () => {
    try {
      ...
      // 3. Tabel Data (Kategori & Jenis ditambahkan)
      ...
      autoTable(doc, {
        ...
        didParseCell: function(data) {
            // Memberi warna pada teks jenis transaksi
      ...
      doc.setTextColor(0).text(`TOTAL PEMASUKAN`, 130, finalY);
      ...
      doc.setDrawColor(200).setLineWidth(0.2).line(14, signY + 36, 60, signY + 36); 

      doc.setFont("helvetica", "bold").text('Bendahara,', 150, signY + 7);
      doc.text('MUH. NUR', 150, signY + 35);
      doc.setDrawColor(200).setLineWidth(0.2).line(150, signY + 36, 196, signY + 36); 
        ...
        <div className="lg:col-span-4">
          <div className="bg-white/[0.03] border border-white/5 p-8 rounded-[2.5rem] sticky top-10">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <button type="submit" disabled={saving} className={`w-full ${formData.jenis_transaksi === 'Masuk' ? 'bg-blue-600' : 'bg-red-600'} text-white font-black uppercase text-xs py-5 rounded-2xl flex items-center justify-center gap-2`}>
                ...
                <tbody className="divide-y divide-white/5">
                  {loading ? (
                    ...
                    <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="p-6">
                        <p className="font-bold text-sm text-white uppercase">{item.nama_pembayar}</p>
                      ...
                      <td className="p-6">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => handleEdit(item)} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg"><Edit3 size={14}/></button>
                          <button onClick={async () => { if(confirm("Hapus?")) { await supabase.from('kas_pb').delete().eq('id', item.id); fetchData(); } }} className="p-2 bg-red-500/10 text-red-400 rounded-lg"><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
