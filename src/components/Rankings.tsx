          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {['All', 'A', 'B+', 'B-', 'C'].map((cat) => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-[10px] font-black border whitespace-nowrap transition-all ${activeCategory === cat ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-900 text-slate-500 border-slate-800 hover:border-slate-600'}`}
              >
                              ...
                              <div className="border-x border-b border-blue-500/20 rounded-b-[2rem] p-6 mb-4 animate-in slide-in-from-top-2 duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                  <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 flex items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs font-black italic text-white uppercase">{player.player_name}</p>
                                    </div>
                                  </div>
                                  <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 flex items-center gap-4">
                                    <div className="p-3 bg-blue-500/10 rounded-xl text-blue-400"><Hash size={20} /></div>
                                    <div>
                                  ...
                                  <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 flex items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs font-black italic text-emerald-500 uppercase flex items-center gap-1"><ShieldCheck size={12} /> Live Sync</p>
                                    </div>
                                  </div>
                                </div>
                                <div className="flex items-center justify-between mb-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Log Aktivitas Terakhir</span>
                                  </div>
                                ...
                                {loadingHistory ? (
                                  <div className="flex flex-col items-center justify-center py-12 gap-3">
                                    ...
                                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-[0.3em]">Mengambil Data Server...</p>
                                  </div>
                                      ...
                                      return (
                                        <div key={log.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isGain ? 'bg-emerald-500/[0.03] border-emerald-500/10 hover:border-emerald-500/30' : 'bg-red-500/[0.03] border-red-500/10 hover:border-red-500/30'}`}>
                                          <div className="flex items-center gap-4">
                                            <div className={`p-2.5 rounded-xl ${isGain ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                                              {isGain ? <ArrowUpRight size={14} strokeWidth={3} /> : <ArrowDownRight size={14} strokeWidth={3} />}
                                            </div>
                                            <div>
                                              <div className="text-[10px] font-mono text-slate-500 mb-0.5">{new Date(log.created_at).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
                                              <div className="flex items-center gap-2">
                                                <span className={`text-[11px] font-black uppercase tracking-tight ${isGain ? 'text-emerald-400' : 'text-red-400'}`}>{isGain ? 'PENAMBAHAN POIN' : 'PENGURANGAN POIN'}</span>
                                                <span className="text-slate-600 text-[10px]">•</span>
                                                <span className="text-[11px] font-bold text-slate-300 uppercase italic">{log.tipe_kegiatan || 'Aktivitas Rutin'}</span>
                                              </div>
                                            </div>
                                          </div>
                                          <div className="text-right">
                                            <div className={`text-xl font-black font-mono ${isGain ? 'text-emerald-400' : 'text-red-400'}`}>{isGain ? '+' : ''}{log.perubahan}</div>
                                            <div className="text-[9px] text-slate-500 font-bold uppercase tracking-tighter flex items-center justify-end gap-1">
                                              <span>Prev: {log.poin_sebelum}</span>
                                              <ChevronRight size={8} />
                                              <span className="text-slate-300">New: {log.poin_sesudah}</span>
                                            </div>
                                          </div>
                                        </div>
                                      );
                                    })}
                                    <div className="text-center pt-4"><p className="text-[8px] font-black text-slate-700 uppercase tracking-widest italic">* Menampilkan 10 riwayat terbaru</p></div>
                                  </div>
                                ) : (
                                  <div className="text-center py-16 border border-dashed border-slate-800 rounded-3xl bg-slate-900/20">
                                    <Calendar className="mx-auto mb-3 opacity-10" size={40} />
                                    <p className="text-slate-600 font-black text-xs uppercase tracking-widest">Riwayat Belum Tersedia</p>
                                  </div>
                                )}
                              </div>
                            </td>
